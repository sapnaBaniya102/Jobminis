...
if (isset($_GET['id'])) {
    # code...
    $siteId = $_GET['id'];
    $query = "SELECT * FROM siteconfig WHERE user_id = '$id' AND id = '$siteId'";
    $result = mysqli_query($conn, $query);
    while ($row = mysqli_fetch_assoc($result)) {
        # code...
$name = $row['name'];
$key = $row['site_key'];
$value = $row['site_value'];
    }
}
 ...
 require('../inc/sidebar.php');
        ...
        <a href="uploadfile.php" class="relative  mx-auto top-16 left-6"> <button class="btn btn-primary p-2">Add</button></a> 
          ...
          <div class="card  rounded-xl">
              <div class="card-header bg-gray-800 rounded-t-xl">
                <h3 class="card-title text-white text-center font-semibold text-lg">Update Site config</h3>
  ...
  <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
      ...
      <input value="<?php echo $name; ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="name" name="name" type="text" placeholder="Jane">
    ...
    <div class="w-full md:w-1/2 px-3">
      ...
      <input value="<?php echo $key; ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="key" name="key" type="text" placeholder="Key">
    </div>
  </div>
  <div class="flex flex-wrap -mx-3 mb-6">
    <div class="w-full px-3">
      ...
      <input value="<?php echo $value; ?>" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="value" name="value" type="text" placeholder="Value">
...
<div id="toast-success" class="flex hidden fixed z-50 right-10 bottom-32 items-center p-4 mb-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
    ...
    <div class="ml-3 text-sm font-normal">Site config updated successfully.</div>
...
<div id="toast-warning" class="flex hidden fixed z-50 right-10 bottom-32 items-center p-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
    ...
    <div class="ml-3 text-sm font-normal">Error while updating site config</div>
...
$value = $_POST['value'];
    ...
    if ($name ==''||$key == ''|| $value=='') {
        ...
        $query = "UPDATE siteconfig SET name = '$name', site_key ='$key', site_value='$value' WHERE user_id = '$id' AND id = '$siteId' ";
        $result = mysqli_query($conn, $query);
        if ($result) {
            # code...
            echo
