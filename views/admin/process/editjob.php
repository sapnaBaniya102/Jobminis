...
if (isset($_GET['id'])) {
    # code...
    $companyId = $_GET['id'];
    $query = "SELECT * FROM job_details WHERE id = '$companyId'";
  ...
  <div class=" flex flex-row flex-wrap">
   ...
   <section class="content w-full ">
      <div class="container-fluid w-4/5 mx-auto">
        ...
        <a href="../app/managejob.php" class="relative  mx-auto top-16 left-6"> <button class="btn btn-primary p-2">Add</button></a> 
...
<div id="toast-success" class="flex hidden fixed z-50 right-10 bottom-32 items-center p-4 mb-4 w-full max-w-xs text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
    ...
    <div class="ml-3 text-sm font-normal">Job updated successfully.</div>
...
$status = $_POST['status'];
    ...
    if ($status =='') {
        ...
        $query = "UPDATE job_details SET status = '$status' WHERE id = '$companyId' ";
        $result = mysqli_query($conn, $query);
        if ($result) {
            # code...
            echo
